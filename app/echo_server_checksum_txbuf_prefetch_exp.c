...
#endif
#ifndef RX_PREFETCH_OFFSET
#define RX_PREFETCH_OFFSET 4
#endif
...
run_server_worker(void *arg)
{
    ...
    printf("TX buffer enabled on queue %u: buffer_size=%u\n",
           queue_id, TX_BUFFER_SIZE);
    printf("RX prefetch enabled on queue %u: offset=%u\n",
           queue_id, RX_PREFETCH_OFFSET);
    ...
    for (;;) {
        ...
        if (nb_rx == 0) {
             ...
             * optional TX-buffer flush, timer reads, and report-check
             * bookkeeping in this path.
             */
        ...
        for (uint16_t i = 0; i < nb_rx; i++) {
            if (i + RX_PREFETCH_OFFSET < nb_rx) {
                rte_prefetch0(
                    rte_pktmbuf_mtod(rx_bufs[i + RX_PREFETCH_OFFSET], void *));
            }
        ...
        if (rte_get_timer_cycles() - last_report >= report_cycles) {
...
parse_echo_args(int argc, char *argv[])
{
    ...
    if (argc < 2 || argc > 3) {
        ...
        printf("usage: sudo ./echo_server_checksum_txbuf_prefetch_exp "
               "[--checksum-mode=software|offload|preserve|none] "
               "<NUM_WORKERS>\n");
        printf("example: sudo ./echo_server_checksum_txbuf_prefetch_exp "
